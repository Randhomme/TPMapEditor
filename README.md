# TPMapEditor

## Get started
Download the latest release (TPMapEditor.zip). On the first opening, the app will ask you to set the TPGame path. Select your TPGame folder (where the game is located), and enjoy map editing !

Found a bug or have a suggestion ? Contact Randhomme on Discord.

## Tips
- You can zoom in and out using CTRL + Mouse wheel
- Something on the map blocks the view ? You can modify it's Z index using +/- button on the top left of the editor (and soon you'll be able to just hide things)
- When rotation is enabled, use ALT + Mouse wheel to rotate selected elements. You can also rotate the preview of an element using ALT + rotate (even if rotation is not enabled)
- You can give color to WaypointPath and WorldPolygon to make them easier to see/recognize (colors are not used in game)
- If you don't find the string you are looking for, you can edit which header file is used for what in App>Settings>Edit game headers

## In the future
I'm sharing this todo list, with things you should expect in next updates
- [ ] Write a complete documentation (values in the map file, values in the map editor)
- [ ] Starmap texture export
- [ ] Add a list of element on latteral bar to show/hide elements
- [ ] Drag and drop selection by drawing a rectangle with the mouse
- [ ] Undo/Redo feature
- [ ] Add mini map export
- [ ] Make the world bigger (5 times should be enough)
- [ ] Add support for saved game
- [ ] Add more app settings
  - [ ] Custom background (image or color)
  - [ ] Dark theme
  - [ ] Custom keyboard shortcut
  - [ ] Hide specific inactive layers
- [ ] Make the WorldRule editor smart, aware of the timeline
- [ ] Make it 3D (not now)
